...
这个文件夹是一份**独立、可以直接部署**的完整拷贝——`index.html`(首页/社区流/我的)/`flow.html`(上传旧衣→生成效果图的核心改造流程，原来叫 `index.html`，⭐2026-09-02改名⭐见下方说明)/`calculator.html` 是从 `MVP流程/` 复制过来的最新版本，`api/generate-creative-image.js` 是新增的服务端云函数。部署之后，MVP 流程里"生成效果图"那一步会真正调用 Gemini API 生成图片，不再是占位卡片。
...
1. 用手机或电脑浏览器打开 `https://你的域名.vercel.app/`(不再是本地文件，是真实网址——顺带这一步也应该会解决之前"拍照选项不显示"的问题，因为那大概率是本地文件权限限制导致的)。⭐2026-09-02更新⭐：根据你的反馈("不要一进来就默认是这个页面，要是首页那个页面")，把首页/社区流页改名成了 `index.html`，原来的核心改造流程页(上传旧衣→...→生成效果图)改名成了 `flow.html`——现在打开网址根路径(`/`)默认就是首页(Vercel 的默认行为是走 `index.html`，现在 `index.html` 就是首页了，不用再手动拼 `/index.html`)，点首页右下角剪刀图标进入改造流程会自动跳到 `flow.html`，两个页面互相之间的跳转链接都已经同步改过，不会出现指向旧文件名的死链接。
...
`MVP流程/flow.html`(原 `index.html`)里的 `CREATIVE_IMAGE_ENDPOINT` 常量已经写的是相对路径 `/api/generate-creative-image`，只要 `flow.html` 和 `api/generate-creative-image.js` 部署在同一个 Vercel 项目/同一个域名下(这份 `部署/vercel/` 文件夹就是这么组织的)，不需要改任何代码，直接就能用。
...
## ⭐2026-09-02新增：首页/我的改造/AI助手对话(`index.html` + `api/assistant-chat.js`，注意：这个页面2026-09-02晚些时候又从 `home.html` 改名成了 `index.html`，见上方说明)

在核心8屏之外新增了一个入口页 `index.html`(首页/发现 · 我的改造 · 我的，底部导航；商城两个位置保留但灰置，点了会提示"这个版本还没做"，不是装作不存在)：

- **首页**：展示项目模板库里13个已验证包型的参考款式卡片(不是别人发的帖子——这个版本没有真实多用户社区)，点一个直接"照着做"，带着这个包型跳进 `flow.html`(`?prefBag=xxx`)，进到选包型那屏会预选好，但依然可以手动改。
- **我的改造**：真实的本地记录，不是摆样子——`flow.html` 在用户选定包型之后的每次切屏都会把进度存进浏览器的 IndexedDB(和 `index.html` 共用同一个数据库/表，同一个域名下天然能互相读到)，"我的改造"里能看到"制作中/已生成纸样/已归档"这几个真实状态，点"续做/查看"能带着之前的数据(包型/风格/照片/已生成过的效果图)跳回 `flow.html`(`?resume=xxx`)接着用。**这一步不需要任何账号系统或服务器**，纯粹是这台设备浏览器本地存的，换设备/清缓存就没了，页面上也如实提示了这一点。
- **我的**：本地统计(改造记录数/已完成数/进行中数，真实从 IndexedDB 算出来的，不是编的)+ 一个只存在这台设备本地的称呼(点"改称呼"可以改，`localStorage` 存的)。粉丝数是仿照高保真设计稿展示效果的示例数字，页面上已如实标注不是真实用户数据。
- **改造小助手**(`api/assistant-chat.js`)：`calculator.html`(制作教程页)和 `index.html` 右下角有个浮动聊天入口，能回答①产品答疑②缝纫知识③结合当前在做的包型给建议(把当前包型名字当背景信息传给模型)。**用的是同一个 `GEMINI_API_KEY`**，不用额外配置；模型和 `identify-garment.js` 一样用 `gemini-3.5-flash-lite`(便宜、够用于问答)，如果部署后发现回答质量不够，可以把这个函数里的 `MODEL_ID` 换成更强的 `gemini-3.7-flash`。这个助手**如实拒绝**它做不到的事——比如用户问"能不能发到社区求助""能不能联系人工客服""能不能查订单"，会照实说这个版本还没有这些功能，不会假装能处理。

⚠️同样的诚实限制：`api/assistant-chat.js` 本身没能在写代码的这台机器上真实联网调用验证过(和这个项目里其他AI功能一样，卡在同一个网络出站限制)，只做过本地 mock 测试(13个断言，覆盖成功/缺key/缺参数/非POST/Gemini报错透传/无文字回复/请求体结构核对)；`index.html` 的首页深链、我的改造记录/续做/归档/删除、AI助手对话框这些前端交互，都用 Playwright 端到端跑通验证过(mock了后端接口)，0个 `pageerror`。真实调用效果如何，需要部署后实际测。

## 这份拷贝和 `MVP流程/`、`纸样计算器/` 的关系

`部署/vercel/index.html`(首页)、`部署/vercel/flow.html`(核心改造流程)、`部署/vercel/calculator.html`(纸样计算器) 是从 `MVP流程/` 复制过来的独立拷贝，为了让 Vercel 能直接部署这一个文件夹。**这三个文件以后如果谁改了(比如核心流程加新功能，或者纸样计算器加新包型)，需要手动把改动同步过来**，不会自动联动——这和交接文档里之前提到过的 `MVP流程/calculator.html` 需要手动从纸样计算器同步是同一类问题，下次改动 `MVP流程/` 下的文件之后记得也 `cp` 一份到这里，重新部署一次(`vercel --prod`)才会生效到真实网址上。
